...
We have created a simulation for detecting DDoS attack using the approach proposed in this paper. As explained in section~\ref{sec:Implementation}, we have a small network created for the demonstration purpose. We captured normal as well as orchestrated DDoS attack traffic in the network. We have processed capture data to convert them into samples and use those samples to create training sets. We are using training sets created from the normal traffic for labeling IP addresses with cluster numbers and then training the classifier.

We have first used elbow method explained in section \ref{subsec:Clustering} on page \pageref{subsec:Clustering} to detect the optimal cluster count. We were able to detect 4 clusters in our training set.
...
We then applied kmeans++ clustering as explained in section \ref{subsec:Clustering} on page \pageref{subsec:Clustering}, to find the correct centroids for our training examples.
...
We then used these centroids to run kmeans++ algorithm and get correct clusters for our training sets.
...
We ran the k-means++ for all the training sets, and produce cluster labels. We then checked how similar the test set clustering is with train set clustering using the Rand Index. We have observed that, with more training sets, the RI index improves.

To have correct clustering we have checked all the cluster assignments for a given destination IP address and selected the cluster which has the highest occurring frequency among all the training sets. We have also count the average number of packets going to a given IP address which will help us reduce the error in detecting the anomaly. We saved this labeling information and cluster count information for each IP destination on the file system.

We then used the training sets which are labeled with cluster number to train the classifier. Classifier would create boundaries around the clusters.

We are using Scikit-learn's `OneClassSVM' library to train the model and create classifier for each cluster. The input vector to the classifier is the set of all the training examples belonging to the same clusters. Thus, for our four clusters, we have four classifier. The input vector to the classifier will be of the form shown in table \ref{table:labeled-set} on page \pageref{table:labeled-set}.
...
Once all the training is done we have file which has IP addresses labeled with cluster number they belong to also we have classifier for each cluster.

Now we take the test set which was created by capturing packets during the DDoS attack and used detection techniques explained in section \ref{}. Using the detection techniques we have successfully detected an attack on the destination IP address 92.168.0.7 in our modeled network.

\section{Mitigation}

By now we have programs which can be run on the router and can be used to detect DDoS attack. As there are many routers through which a destination can be reached, so to correctly detect DDoS attack we should have these programs on every router in the network or all the routers in the Internet. One of the way to achieve this is by using Network function virtualization (NFV)\cite{nfv}. NFV is an advanced technology which allows to manage network devices remotely and also allows to add functionality to those network devices. Because of this technique we don't have to manually install computer programs on the routers rather we can do it any time remotely. Although this technology is new but its spreading very fast and soon will be available for every commercial router.

After detecting an attack, the router can choose to block all the packets going to the destination under attack or it can communicate the attack information to the destination, destination could be a server or a router. The attack information can contain region and packet count. Region is the geographical location where the router is situated and packet count is the number of packets observed at the router having a given destination. Communicating the attack information to destination can be more helpful, because the destination will be aware of the nature of the attack, i.e. from where it is originated and how intense the attack is. Destination can also query router for more information such as IP source of the attack packets. The control of blocking traffic at router can also be given to the destination. In this case, destination can perform its own analysis, understand the nature of attack and then can decide, whether to block incoming packets on the router. There can be different parameter based on which the destination server can decide blocking the flow. Many questions can be asked before making the decision, such as, how many routers have reported the change in traffic?, Is the attack information coming from the region which never had traffic in the past?
